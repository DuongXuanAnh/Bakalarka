...
export default function AddDependency() {
  ...
  const handleAddDependency = useCallback(() => {
    ...
    setDependencies((prev) => [
      ...prev,
      { left: leftAttributes, right: rightAttributes },
    ]);
    setLeftAttributes([]);
    setRightAttributes([]);
  ...
  const handleRemoveAttribute = useCallback((side, indexToRemove) => {
    const setter = side === "left" ? setLeftAttributes : setRightAttributes;
    setter((prevAttributes) =>
      prevAttributes.filter((_, index) => index !== indexToRemove)
    );
  }, []);

  const renderComboBoxes = useCallback(
    (side) => {
      ...
      return (
        <>
          {attributesList.map((selectedAttribute, index) => (
            <div key={index} className="comboBoxWrapper">
              <span className="attributeTag">
                {selectedAttribute}
                <button
                  onClick={() => handleRemoveAttribute(side, index)}
                  className="removeBtn"
                >
                  x
                </button>
          ...
          >
            ...
            {attributes
              .filter((attr) => !attributesList.includes(attr))
              .map((attr) => (
  ...
  return (
    <div className="addDependencyContainer">
      ...
      <button onClick={handleAddDependency} className="addBtn">
        {t("content-dependencies.addRelation")}
      </button>
      <button onClick={goBack} className="backBtn">
        {t("content-dependencies.back")}
      </button>
    </div>
  );
}
